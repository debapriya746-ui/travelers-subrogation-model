...
found during EDA:
  ...
  * Vehicle-made-year "time travel" glitch      -> invalid_vehicle_year flag,
                                                    vehicle_age kept as-is (still
                                                    predictive despite being wrong)
  * License-age issue                           -> invalid_license_age flag
  * Driver-age anomalies (implausible values)   -> driver_age capped to [15, 100]
  * Claim-day-of-week mismatch                  -> day of week recomputed from
                                                    claim_date rather than trusting
                                                    the provided field
  * Financial risk ratios & threshold flags     -> vehicle/claim/income ratios,
...
learned in `fit` on the training fold only, so there's no leakage).
"""
...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    driver_age_bounds : tuple(int, int)
        ...
        (the raw data contains outliers well outside a plausible human age).
    """
    ...
    def fit(self, X, y=None):
        ...
        # Quantile bin edges learned on the training fold only, then reused
        # as fixed pd.cut edges in transform() - pd.qcut recomputes quantiles
        # from whatever data it's given, so calling it fresh inside transform
        # would make train/validation/test bins inconsistent and would return
        # all-NaN for a single-row or small-batch prediction.
        _, income_edges = pd.qcut(X_temp["annual_income"], q=5, retbins=True, duplicates="drop")
        _, claim_edges = pd.qcut(X_temp["claim_est_payout"], q=5, retbins=True, duplicates="drop")
        # Open-ended outer edges so values outside the training range (e.g.
        # at inference time) fall into the nearest bin instead of NaN.
        income_edges[0], income_edges[-1] = -np.inf, np.inf
        claim_edges[0], claim_edges[-1] = -np.inf, np.inf
        self.income_bin_edges_ = income_edges
        self.claim_bin_edges_ = claim_edges
        return self
    ...
    def transform(self, X):
        ...
        # Cap driver age at [15, 100] per the data-quality review
        X["driver_age"] = X["driver_age_raw"].clip(*self.driver_age_bounds)

        X["vehicle_age"] = X["claim_date"].dt.year - X["vehicle_made_year"]
        # Vehicle made-year "time travel" glitch: made-year is sometimes in the
        # future relative to the claim. We flag it rather than drop it: despite
        # being impossible, vehicle_age retained real signal.
        X["invalid_vehicle_year"] = (X["vehicle_made_year"] > X["claim_date"].dt.year).astype(int)

        # License-age impossibility: license obtained before the driver
        ...
        # Claim-date/day-of-week mismatch: recompute day of week from
        ...
        # --- Binned features ---------------------------------------------
        X["income_bin"] = pd.cut(X["annual_income"], bins=self.income_bin_edges_, labels=False, include_lowest=True)
        X["claim_size_bin"] = pd.cut(
            X["claim_est_payout"], bins=self.claim_bin_edges_, labels=False, include_lowest=True
        )
        # Liability quartiles: the single strongest predictor:
